...
trace(
  `name: ${preferences.name}, city: ${preferences.city}, state: ${
    preferences.state
  }\n`
)
file.close(file)
...
for (let i = 0; i < length; ++i) shorts[i] = i
...
for (let i = 0; i < length; ++i) trace(shorts[i] + ' ')
file.close(file)
trace('\n')
...
while ((item = iterator.next())) {
  if (undefined == item.length) trace(`${item.name.padEnd(32)} directory\n`)
  else trace(`${item.name.padEnd(32)} file          ${item.length} bytes\n`)
}
trace('\n')
